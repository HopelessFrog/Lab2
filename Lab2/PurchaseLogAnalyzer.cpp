...
std::vector<PurchaseLog*> ReadLogFromConsole() {
    ...
    int rows = GetInt();
    std::cout << "Please enter each log entry in the format:\n"
        << "YYYY-MM-DD Brand Mileage PricePerGallon Gallons TotalCost" << std::endl;
    for (int i = 0; i < rows;) {
        std::string line;
        ...
        try {
            ...
            std::cout << BOLD << RED <<  "Inputted string contains incorrect data, input again" << std::endl;
            continue;
        }
        logs.push_back(line);
        i++;
...
std::vector<PurchaseLog*> ReadLogFromFile(std::string path) {
    ...
    if (!file.is_open()) {
        std::cout << BOLD << RED << "File at the path does not exist" << std::endl;
        return result;
    }
    std::string line;
    std::vector<std::string> buffer;
    ...
    if (isCorrupted) {
        std::cout << std::endl << BOLD << RED << "Some of the file data is corrupted." << std::endl;
    }
    return result;
}
...
void SaveToFileLogs(std::vector<std::string>& logs) {
    ...
    while (true)
    {
        ...
        {
            std::cout << BOLD << RED << "Incorrect file name, try again" << std::endl;
            continue;
        }
        ...
        if (check.is_open())
        {
            ...
            if (rewrite == "yes")
            {
                ...
                if (!input.is_open())
                {
                    std::cout << BOLD << RED << "This file is read-only" << std::endl;
                    std::cout << "Input file name" << std::endl;
                    continue;
                }
                if (!std::filesystem::is_regular_file(fileName))
                {
                    std::cout << BOLD << RED << "This file name is not allowed" << std::endl;
                    input.close();
                ...
                std::cout << GREEN << "Data is saved" << std::endl;
                break;
            }
            else {
                std::cout << YELLOW << "Input another filename" << std::endl;
            }
        }
        else
        {
            ...
            std::cout << BOLD << GREEN << "Data is saved" << std::endl;
            break;
        }
    }
}
